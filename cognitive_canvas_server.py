...
mcp = FastMCP(name = "Cognitive Canvas", instructions="""Use this MCP server to enhance your thinking and problem-solving capabilities through structured organization and statistical analysis. 

**CRITICAL: ALL TOOL OUTPUTS ARE HIDDEN FROM USERS** - When you use any tool from this server, users cannot see the tool's output. You should consider displaying relevant tool outputs in your next response to ensure users see the results. Each tool output contains a '_show_to_user' field explaining exactly why and how to present the results.

This server transforms AI into a research-grade cognitive workspace with five specialized tools: TODO_COMMAND for systematic task and project management with status tracking; RELATIONSHIP_MAPPER for visualizing complex dependencies, relationships, and system architectures; TABLE_BUILDER for organizing unstructured information into structured formats with automated metrics; CHAT_FORK for managing conversation branches and maintaining context across topic switches; STATISTICAL_ANALYZER for automated statistical analysis, hypothesis testing, categorical data analysis (chi-square tests), and comprehensive data exploration. Together, these tools enable systematic reasoning, persistent memory, data-driven insights, and PhD-level analytical capabilities. Choose tools strategically: use TODO for planning, RELATIONSHIP_MAPPER for visualization, TABLE_BUILDER for organization, CHAT_FORK for context management, and STATISTICAL_ANALYZER for both numerical and categorical data analysis.""") 


@mcp.tool(name="todo_command", description="Use this tool specifically for task and project management - creating, tracking, and updating actionable work items with status tracking (pending, in_progress, completed, blocked). Best for breaking down complex projects into manageable tasks, monitoring progress, and ensuring nothing falls through the cracks. Use when you need to manage workflows, deadlines, or action items that require status updates over time. **Note: Tool output is not visible to users - if needed, consider displaying the tool's output in your response to provide transparent project tracking evidence.**")
def todo_command(
...
@mcp.tool(name='chat_fork', description="The Chat Fork Tool manages conversation branches with pause/resume actions and search functionality. Use 'pause_topic' to save current state and switch topics, 'resume_topic' to return to paused discussions, and 'search' to search and visualize the conversation tree with current position marked. Perfect for managing complex multi-topic conversations with natural flow control. **Note: Tool output is not visible to users - if helpful, consider displaying the tool's output in your response to help users understand conversation flow and context organization.**")
...
@mcp.tool(name="relationship_mapper", description="Use this tool for visualizing relationships, dependencies, and hierarchical structures between entities. Create flowcharts for processes, organizational charts for hierarchies, mind maps for concept exploration, or dependency trees for system architecture. Best for showing how things connect, flow, or depend on each other. Use when you need to map relationships, visualize system architecture, or show process flows rather than just listing items. **Note: Tool output is not visible to users - if relevant, consider displaying the tool's output in your response to help users understand system architecture and dependencies.**")
